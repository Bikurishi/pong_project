        ...
        function draw() {
            ...
            if (gameState === 'secret_transition') {
                const p = secretTransitionTimer / 180; 
                ctx.save();
                ctx.translate(600, 300);
                if (canvas.getBoundingClientRect().height > canvas.getBoundingClientRect().width) ctx.rotate(Math.PI / 2);
                ctx.fillStyle = "white"; ctx.font = "bold 60px Segoe UI"; ctx.textAlign = "center"; ctx.fillText("VICTORY", 0, 0);
                ctx.restore();
